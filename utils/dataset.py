...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        mask_label1 = np.zeros(shape=(h, w), dtype=np.int)
        mask_label2 = np.zeros(shape=(h, w), dtype=np.int)
        mask_label2[:, :] = 1
        ...
        return {'image': torch.from_numpy(img), 'mask': torch.from_numpy(mask_label),
                'mask1': torch.from_numpy(mask_label1), 'mask2': torch.from_numpy(mask_label2)}
